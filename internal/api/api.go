...
type serverResponse struct {
	...
	Servers []proxy.ProxyServer `json:"servers"`
...
func (a *API) cacheBase64(w http.ResponseWriter, r *http.Request) {
	...
	servers := proxy.FilterCountries(a.service.Cached(false), r.URL.Query().Get("country"))
...
func (a *API) cacheAllBase64(w http.ResponseWriter, r *http.Request) {
	...
	writeSubscription(w, proxy.FilterCountries(all, r.URL.Query().Get("country")), true)
}
func (a *API) siteSpecific(w http.ResponseWriter, r *http.Request) {
	...
	servers = proxy.FilterCountries(servers, r.URL.Query().Get("country"))
	if len(servers) == 0 {
...
func (a *API) testCustom(w http.ResponseWriter, r *http.Request) {
	...
	if req.TestURL != "" && !proxy.ValidTargetURL(req.TestURL) {
...
func (a *API) sites(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if !proxy.ValidTargetURL(req.URL) {
			writeError(w, http.StatusBadRequest, "url must be a valid http(s) URL")
			return
		}
		if req.Filename == "" {
			req.Filename = config.SitesFromURLs([]string{req.URL})[0].Filename
		}
		if _, err := service.SafeOutputName(req.Filename); err != nil {
			writeError(w, http.StatusBadRequest, "filename must be a safe relative path")
			return
		}
		enabled := true
		if err := a.service.PutSite(r.Context(), config.SiteConfig{URL: req.URL, Filename: req.Filename, Enabled: &enabled}); err != nil {
			writeError(w, http.StatusServiceUnavailable, err.Error())
			return
		}
		writeJSON(w, http.StatusCreated, map[string]any{"site": config.SiteConfig{URL: req.URL, Filename: req.Filename, Enabled: &enabled}, "message": "site stored; it will be prechecked during refresh and is available through /subscription/site-specific"})
...
func (a *API) authorizeManagement(w http.ResponseWriter, r *http.Request) bool {
	expected := a.service.ManagementToken()
	if expected == "" {
...
func writeTestResponse(w http.ResponseWriter, servers []proxy.ProxyServer, err error) {
	if errors.Is(err, service.ErrBusy) {
		writeError(w, http.StatusTooManyRequests, err.Error())
		return
	}
...
func requireCache(w http.ResponseWriter, servers []proxy.ProxyServer) bool {
	if len(servers) == 0 {
...
func writeSubscription(w http.ResponseWriter, servers []proxy.ProxyServer, encoded bool) {
