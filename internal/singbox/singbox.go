...
// DefaultPath returns the preferred sing-box location: explicit env,
// system install, or the user data dir (auto-provisioned on demand).
func DefaultPath() string {
	if v := os.Getenv("SING_BOX_PATH"); v != "" {
		return v
	}
	if runtime.GOOS == "windows" {
		return filepath.Join(xdg.DataDir(), "bin", "sing-box.exe")
	}
	...
	return filepath.Join(xdg.DataDir(), "bin", "sing-box")
}
...
func ResolveSingBox(configured string) string {
	...
	candidates = append(candidates, DefaultPath(), "/usr/local/bin/sing-box", "/usr/bin/sing-box")
