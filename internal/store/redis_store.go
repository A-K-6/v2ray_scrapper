...
func (s *RedisStore) SeedAndLoad(ctx context.Context, subscriptions []string, sites []config.SiteConfig) ([]string, []config.SiteConfig, error) {
	if s == nil {
		return append([]string(nil), subscriptions...), append([]config.SiteConfig(nil), sites...), nil
	}
	...
	loadedSites := make([]config.SiteConfig, 0, len(storedSites))
...
func (s *RedisStore) PutSite(ctx context.Context, site config.SiteConfig) error {
	if s == nil {
		return fmt.Errorf("Redis is not configured")
	}
	data, err := json.Marshal(site)
...
func (s *RedisStore) GetSiteCache(ctx context.Context, target string) ([]proxy.ProxyServer, bool, error) {
	if s == nil {
...
func (s *RedisStore) SetSiteCache(ctx context.Context, target string, servers []proxy.ProxyServer, ttl time.Duration) error {
	if s == nil {
		return nil
	}
