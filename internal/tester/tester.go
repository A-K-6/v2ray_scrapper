...
type Tester struct{ config config.Config }

func NewTester(config config.Config) *Tester { return &Tester{config: config} }

func (t *Tester) Test(ctx context.Context, servers []proxy.ProxyServer, target string, siteCheck bool) []TestResult {
	if len(servers) == 0 {
		return nil
	}
	type batch struct {
...
func (t *Tester) testBatch(ctx context.Context, servers []proxy.ProxyServer, target string, siteCheck bool, basePort int) []TestResult {
...
func buildSingBoxConfiguration(servers []proxy.ProxyServer, basePort int) (map[string]any, int, error) {
...
func (t *Tester) testBuildableBatch(ctx context.Context, servers []proxy.ProxyServer, target string, siteCheck bool, basePort, invalid int, cause error) []TestResult {
	...
	valid := make([]proxy.ProxyServer, 0, len(servers)-1)
...
func testSOCKSProxy(ctx context.Context, port int, target string, timeout time.Duration, siteCheck bool, attempts int) int {
	dialer, err := netproxy.SOCKS5("tcp", fmt.Sprintf("127.0.0.1:%d", port), nil, &net.Dialer{Timeout: timeout})
	if err != nil {
		return -1
	}
...
func failedResults(servers []proxy.ProxyServer) []TestResult {
	result := make([]TestResult, len(servers))
