...
func TestTesterReturnsFailureWhenSingBoxCannotStart(t *testing.T) {
	tester := NewTester(config.Config{SingBoxPath: "/definitely/missing/sing-box", TestTimeout: time.Millisecond, SingBoxStartTimeout: time.Millisecond, BatchSize: 10, MaxConcurrentBatches: 1, BasePort: 20000})
	servers := []proxy.ProxyServer{{Protocol: "vless", Address: "1.2.3.4", Port: 443, ID: "bf000d23-0752-40b4-affe-68f7707a9661"}}
...
func TestFailedResultsPreservesServers(t *testing.T) {
	servers := []proxy.ProxyServer{{Address: "one"}, {Address: "two"}}
