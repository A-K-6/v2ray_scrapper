...
func (s *Scraper) FetchAll(ctx context.Context, urls []string) []proxy.ProxyServer {
...
sendJobs:
	...
	wg.Wait()
	close(results)
	bySource := make([][]proxy.ProxyServer, len(urls))
	...
	merged := make([]proxy.ProxyServer, 0)
	for offset := 0; ; offset++ {
		added := false
...
func (s *Scraper) Fetch(ctx context.Context, source string) ([]proxy.ProxyServer, error) {
	...
	return proxy.ParseSubscription(string(body)), nil
}
