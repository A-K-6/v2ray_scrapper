# Idea: Refactor Xray Execution (DRY)
...
1.  Create a class `XrayProcessContext` (async context manager).
...
2.  Refactor `run_test_batch` to use:
    ```python
    async with XrayProcessContext(config, ports) as xray:
        # Run tests
    ```
3.  Refactor `evaluate_site_accessibility` to use the same context manager.
