...
func exportServers(servers []ProxyServer, format, countries string, limit int, out string) int {
	servers = filterCountries(servers, countries)
...
func runSites(args []string) int {
	...
	case "add":
		...
		if filename == "" {
			filename = sitesFromURLs([]string{url})[0].Filename
		}
		enabled := true
		if err := svc.PutSite(ctx, SiteConfig{URL: url, Filename: filename, Enabled: &enabled}); err != nil {
...
func runToken(args []string) int {
	...
	case "show":
		...
		if tok == "" {
			if t := readEnvToken(filepath.Join(configDir(), ".env")); t != "" {
				tok = t
...
func runConfigInit(args []string) int {
	...
	if err := ensureDir(filepath.Join(dataDir(), "bin")); err != nil {
		fmt.Fprintf(os.Stderr, "mkdir: %v\n", err)
		return 1
	}
	...
	hasToken := strings.TrimSpace(os.Getenv("MANAGEMENT_TOKEN")) != "" ||
		readEnvToken(filepath.Join(configDir(), ".env")) != ""
	if !hasToken {
...
func runDoctor(args []string) int {
	...
	sb := resolveSingBox(config.SingBoxPath)
	if isExecutable(sb) {
		pass("sing-box: %s", sb)
	} else {
	...
	if config.RedisURL != "" {
		...
		pass("standalone registry: %s", registryPath())
	}
	...
	state, _ := NewStateStore(config.StatePath).Load()
...
func tuiSites(in *bufio.Reader) {
	...
	case "a", "add":
		...
		enabled := true
		fn := sitesFromURLs([]string{u})[0].Filename
		if err := svc.PutSite(ctx, SiteConfig{URL: u, Filename: fn, Enabled: &enabled}); err != nil {
			fmt.Fprintf(os.Stderr, "add: %v\n", err)
			return
		}
		fmt.Printf("added -> %s\n", fn)
